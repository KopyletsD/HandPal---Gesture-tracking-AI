...
class TutorialManager:
    ...
    def check_step_completion(self, results):
        ...
        if completion_type == "position" and self.handpal.last_right_hand_lm_norm is not None:
            ...
            active_gesture = self.handpal.gesture_recognizer.gesture_state.get("active_gesture", None)
            
            if required == active_gesture :
                self.step_completed = True
...
class HandPal:
    ...
    def draw_overlays(self, frame, results):
        ...
        # --- Normal / Debug Overlay --- (Unchanged)
            ...
            if self.debug_mode:
                panel_h=110; bg=frame.copy(); cv.rectangle(bg,(0,h-200-panel_h),(w,h-200),bg_c,-1); alpha=0.7
                frame=cv.addWeighted(bg,alpha,frame,1-alpha,0); y=h-panel_h-200+15; d=self.debug_values; m=d['map']; c=self.config['calibration']
