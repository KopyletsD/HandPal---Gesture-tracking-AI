...
class Config:
    ...
    "click_cooldown": 0.4,
    "gesture_sensitivity": 0.05,  # Valore ridotto per richiedere un contatto quasi totale
...
class TutorialManager:
    def __init__(self, handpal):
        ...
        # Definisci tutti i passaggi del tutorial
        self.steps = [
            {
            ...
            },
            {
                ...
                "required_gesture": "click",
                "icon": "👆"
            },
            {
                "title": "Doppio Click",
                "instruction": "Avvicina pollice e indice rapidamente due volte",
                "completion_type": "gesture",
                "required_gesture": "double_click",
                "icon": "✌"
            },
            {
                "title": "Scorrimento",
    ...
    def check_step_completion(self, results):
        ...
        if completion_type == "position" and self.handpal.last_right_hand_lm_norm is not None:
            ...
            if x < margin and y < margin:
                current["corners_visited"][0] = True
            # Top-right
            if x > screen_w - margin and y < margin:
                current["corners_visited"][1] = True
            # Bottom-right
            if x > screen_w - margin and y > screen_h - margin:
                current["corners_visited"][2] = True
            # Bottom-left
            if x < margin and y > screen_h - margin:
                current["corners_visited"][3] = True
                
            if all(current["corners_visited"]):
                self.step_completed = True
                
        elif completion_type == "gesture":
            required = current.get("required_gesture", "")
            active_gesture = self.handpal.gesture_recognizer.gesture_state.get("active_gesture")
            
            if required == "click" and active_gesture == "click":
                self.step_completed = True
            elif required == "double_click" and self.handpal.debug_values.get("gest") == "double_click":
                self.step_completed = True
            elif required == "scroll" and active_gesture == "scroll":
                # For scroll we need to detect actual scroll movement
            ...
            # Check if menu trigger is active and timer
    ...
    def draw_tutorial_overlay(self, frame):
        ...
        # Create semi-transparent overlay at bottom of screen
        overlay_height = 120
...
# -----------------------------------------------------------------------------
# Menu Related Functions & Classes (Modified with Tutorial)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# HandPal Class (Main Application - Modified for Tutorial)
# -----------------------------------------------------------------------------
class HandPal:
    def __init__(self, config):
    ...
    # Core Methods (omitted for brevity, assume they're identical to original code)
    def start(self):
...
# -----------------------------------------------------------------------------
# Main Function (Modified for Tutorial)
# -----------------------------------------------------------------------------
def main():
    ...
    try:
        app = HandPal(config)
        # Rendi l'istanza disponibile globalmente per il tutorial
        global handpal_instance
        handpal_instance = app
