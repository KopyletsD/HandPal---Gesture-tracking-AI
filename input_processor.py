...
# REMOVE THESE GLOBAL VARIABLES - they are causing the conflict
# _menu_trigger_enter_time = 0
# menu_trigger_hovering = False
...
class HandInputProcessor:
    def __init__(self, config, smoother: MotionSmoother, gesture_recognizer: GestureRecognizer):
        ...
        # Initialize this to False since we're not hovering at startup
    ...
    def _check_menu_trigger(self, menu_zone_px, display_dims):
        """
        Check if the hand is hovering over the menu trigger zone.
        Returns True if the menu should be activated, False otherwise.
        """
        # First check if we have a valid menu zone
        if not menu_zone_px or not menu_zone_px.get("is_valid", False):
            if self.menu_trigger_hovering:
                print("Menu trigger deactivated (Invalid Zone).")
                logger.debug("Menu trigger deactivated (Zone Invalid).")
                self.menu_trigger_hovering = False
                self._menu_trigger_enter_time = 0
            self.debug_info["menu_check"] = "Off (Zone Invalid)"
            return False

        # Check if we have a valid hand position
        ...
        if hand_pos_display_px is None:
            if self.menu_trigger_hovering:
                print("Menu trigger deactivated (Hand Lost).")
                logger.debug("Menu trigger deactivated (Hand Lost).")
                self.menu_trigger_hovering = False
                self._menu_trigger_enter_time = 0
            self.debug_info["menu_check"] = "Off (No Hand)"
            return False

        # Calculate distance to menu center
        hx_px, hy_px = hand_pos_display_px
        cx_px = menu_zone_px["cx"]
        cy_px = menu_zone_px["cy"]
        radius_sq_px = menu_zone_px["radius_sq"]
        self.debug_info["menu_zone_r_sq"] = radius_sq_px

        dist_sq_px = (hx_px - cx_px)**2 + (hy_px - cy_px)**2
        is_inside = dist_sq_px < radius_sq_px
        self.debug_info["menu_dist_sq"] = dist_sq_px

        now = time.time()
        activate_now = False

        # If hand is inside the menu zone
        if is_inside:
            # Set entry time ONLY when first entering (not already hovering)
            if not self.menu_trigger_hovering:
                self._menu_trigger_enter_time = now
                print(f"[ENTER] Hover started at {self._menu_trigger_enter_time:.2f}")
                logger.debug(f"[ENTER] Hover started at {self._menu_trigger_enter_time:.2f}")
                self.menu_trigger_hovering = True
            
            # Calculate the current hover duration
            hover_time = now - self._menu_trigger_enter_time
            self.debug_info["menu_check"] = f"Hover {hover_time:.1f}s"
            
            # Display debug info
            print(f"[HOVERING] Time: {hover_time:.1f}s [DELAY] Time: {self.MENU_HOVER_DELAY}")
            logger.debug(f"[HOVERING] Time: {hover_time:.1f}s of {self.MENU_HOVER_DELAY}s required")
            
            # Check if hover threshold is reached
            if hover_time >= self.MENU_HOVER_DELAY:
                activate_now = True
                print(f"[ACTIVATE] Triggering menu!")
                logger.debug("[ACTIVATE] Triggering menu!")
        else:
            # Reset if moved outside the zone
            if self.menu_trigger_hovering:
                print("Menu trigger deactivated (Moved Outside).")
                logger.debug("Menu trigger deactivated (Moved Outside).")
                self.menu_trigger_hovering = False
                self._menu_trigger_enter_time = 0
    ...
    def process(self, detection_results, proc_dims, display_dims, menu_trigger_zone_px):
        ...
        if output['landmarks_r'] and self.last_right_hand_lm_norm:
            ...
            output['cursor_smooth_px'] = self.smoother.last_smooth_pos
            # Reset hover state if hand is lost
            if self.menu_trigger_hovering:
                logger.debug("Menu trigger reset (Right Hand Lost).")
                self.menu_trigger_hovering = False
                self._menu_trigger_enter_time = 0
            self.debug_info["menu_check"] = "Off (No R Hand)"
            output['calib_norm_pos'] = None
